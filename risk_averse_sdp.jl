...
#
# functions for Risk averse Stochastic Dynamic Programming 

using ProgressMeter
using StatsBase

function risk_averse_value_functions(λ, α, train_noise, controls, states, dynamics, interpolation, price, T)

	"""compute risk averse value functions: return (T+1, length(states)) array
    λ > weight of risk measure (1-λ)E[z]+λAV@R[z]
    α > P[z<V@Rα] = 1-α
    ...
    @showprogress for t in T:-1:1
        ...
        var_α = quantile(noise, AnalyticWeights(p_noise), 1-α)
        p_t = price[t]
        ...
        for x in states
            ...
            for i in 1:1:length(noise)
                ...
                expectation += v*p_w
                avar += max(0, v-var_α)*p_w
                
            end
            
            value_function[t, xdict[x]] = (1-λ)*expectation + λ*(var_α + avar/α)
